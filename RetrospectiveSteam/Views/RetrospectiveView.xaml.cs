...
namespace RetrospectivaAnual.Views
{
    public partial class RetrospectiveView : UserControl
    {
        ...
        private void UserControl_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            {
                var vm = DataContext as RetrospectiveViewModel;
                if (vm != null)
                {
                    // Threshold: If width is less than 1500px, use 4 columns, otherwise use 5.
                    // This ensures that on 1080p (approx 1920px width, but split in two columns) 
                    // we show 4 items per row for better readability. 
                    // 5 items will only appear on very large resolutions (like 4K).
                    vm.TimelineColumns = e.NewSize.Width < 1500 ? 4 : 5;
                }
            }
        }
    }
    ...
    public class MultiplyConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            ...
            double factor = SafeParseDouble(values[1]);
